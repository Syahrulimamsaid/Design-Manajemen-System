...
interface DropdownProps {
  id: string;
  selectDesigner: (value: string[]) => void;
}

const MultiSelect: React.FC<DropdownProps> = ({ id, selectDesigner }) => {
  ...
  useEffect(() => {
    if (getDataDesigner == null) {
      handleGetDesigner();
    }
    selectDesigner(selected.map((select) => options[select].value));
    console.log(selected.map((select) => options[select].value));
  }, [id, selected]);
    ...
    <div className="relative z-50">
      ...
      <div>
        <select className="hidden" id={id}>
          {getDataDesigner && getDataDesigner.length > 0
            ? getDataDesigner.map((designer, index) => (
                <option key={index} value={designer.kode}>
                  {designer.nama}
                </option>
              ))
            : ''}
