...
import { detailQC } from '../../../Helpers/API/DashboardAPI';
...
const ChartQC: React.FC = () => {
  const [month, setMonth] = useState<string>('1');

  const [detailQCData, setDetail] = useState<any>({
    reject: [],
    acc: [],
  });
  ...
  const handleDetailJob = async () => {
    try {
      const detail = await detailQC(token);
  ...
  useEffect(() => {
    ...
    const filterDataByMonth = (monthIndex: number) => {
      ...
      return {
        reject: [detailQCData.reject[index]],
        acc: [detailQCData.acc[index]],
      };
    };
    ...
    const seriesData = [
      {
        name: 'Acc',
        data: filteredData.acc,
      },
      {
        name: 'Rejec',
        data: filteredData.reject,
      },
    ];
